# BeansMarket - 구디 78기 세미프로젝트

## 팀 소개

|      이도혜       |          김정언         |       장성영         |
| :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
|   <img width="160px" src="https://github.com/JangSeongYeong/BeansMarket/assets/109867833/512d0efb-ac18-4e78-a828-5684ecfc14f4" />    |                      <img width="160px" src="https://github.com/JangSeongYeong/BeansMarket/assets/109867833/512d0efb-ac18-4e78-a828-5684ecfc14f4" />    |                  <img width="160px" src="https://github.com/lee-seong-wook/MyLock/assets/130055880/cf67e022-41c2-45d4-94ab-dff07c089034"/>   |
| 팀장 | 팀원 | 팀원 |
|  웹 디자인 <br/> 검색 서비스<br/> 고객 서비스    |  게시글 서비스 <br/> 서류 작성    | 거래 서비스 <br/> 채팅 서비스 <br/> 경매 서비스  |

<br/><br/>

## 프로그램 소개
<p>구디 아카데미 78기 세미 프로젝트 - 중고 거래 홈페이지</p>
<img width="750px" src="https://github.com/JangSeongYeong/BeansMarket/assets/109867833/eb0f8fb8-48b2-4d69-876c-fc397e8cd8b7"/>
<br/>

## 수행 업무
* 게시글 상세 보기 페이지 개발
  - 일반 판매 게시글과 경매 게시글의 페이지를 구분해서 작성했습니다.
  - mybatis 라이브러리를 사용해 DB에 저장되어 있는 게시글 정보, 연관된 사진 데이터 가져와 페이지에 표시되도록 하였습니다.
  - 신고, 찜하기 기능들을 Ajax를 통한 비동기로 처리되도록 하였습니다.
* 쪽지 시스템 개발 (대화방 형식, 실시간 화면 반영 X)
  - 상세 보기에서 쪽지 보내기로 이동시에 특정 게시물과 인원들이 연락 중인 쪽지 리스트 출력합니다.
  - 게시물과 대화 상대방을 구분하여 연락 리스트 및 마지막 대화내용 출력합니다.
  - 본인에게만 보여지지 않도록 쪽지 삭제를 구현했습니다.
  - 사진 전송 기능을 추가했습니다.
  - 신고 기능을 추가했습니다.
  - 거래 처리 시스템 동작 처리 폼 추가했습니다.
* 경매 시스템 개발
  - 경매 게시글 상세 페이지에서 경매 시스템 동작합니다.
  - 실시간으로 처리
  - 
* 거래 처리 시스템 개발
* 페이 관리 페이지 개발
* 관리자 페이지 디자인
* 쪽지 관리 페이지 개발
* 신고 관리 페이지 개발
* 전체적인 히스토리 검색 페이지 개발

<br/><br/>

## 프로젝트 일정 (제작 기간)
<p>4월 1일 ~ 5월 10일 (40일)</p>
<img width="750px" src="https://github.com/JangSeongYeong/BeansMarket/assets/109867833/b55d5066-4ea6-45f7-b3ff-c735e1527788" />

<br/><br/>

## 사용 툴 구성
<img width="750px" src="https://github.com/JangSeongYeong/BeansMarket/assets/109867833/2e4e854e-c02e-474a-801a-d32341d3d9a1" />

<br/><br/>

## 기능정의서
https://drive.google.com/drive/folders/1yBIdBRZZfsOw1D5pBxf8c4Sx64ZoM7zI
<br/><br/>

## ERD 설계
<img width="750px" src="https://github.com/JangSeongYeong/BeansMarket/assets/109867833/350a91d0-93fc-4ba6-aff0-6a5cc3304fe4" />
<br/><br/>

## UI 설계
https://drive.google.com/drive/folders/1yBIdBRZZfsOw1D5pBxf8c4Sx64ZoM7zI
<br/><br/>

## 인터페이스 정의서
https://drive.google.com/drive/folders/1yBIdBRZZfsOw1D5pBxf8c4Sx64ZoM7zI
<br/><br/>

## 테스트 내역서
https://drive.google.com/drive/folders/1yBIdBRZZfsOw1D5pBxf8c4Sx64ZoM7zI
<br/><br/>

## 기능 소개

